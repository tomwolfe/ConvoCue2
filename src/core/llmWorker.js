...
let shortTermMemory = []; // Keep last 10 messages internally
        ...
        switch (type) {
            ...
            case 'llm':
                ...
                if (messages && messages.length > 0) {
                    ...
                    shortTermMemory = messages.slice(-10);
                ...
                const summaryStr = conversationSummary ? ` Summary so far: ${conversationSummary}.` : '';

                const systemPrompt = `Role:${context.persona}. Intent:${context.intent}.${recentIntentsStr}${summaryStr} Battery:${context.battery}%. Goal:${instruction}. Context: Provide a relevant, concise suggestion based on the conversation history.`;
...
- If exhausted, suggest exit strategies\`\`\n` +
            ...
            case 'summarize':
                ...
                const summaryPrompt = `Analyze this conversation transcript and stats to provide a concise social battery summary and a one-sentence rolling summary.
...
[One sentence summarizing the main topic and mood for AI context]`;
                ...
                const rawResult = summaryOutput[0].generated_text.trim();

                

                // Parse the result

                let summaryPoints = rawResult;

                if (rawResult.includes('ROLLING_SUMMARY:')) {

                    const parts = rawResult.split('ROLLING_SUMMARY:');

                    summaryPoints = parts[0].replace('SUMMARY_POINTS:', '').trim();

                    conversationSummary = parts[1].trim();

                }



                self.postMessage({ type: 'summary_result', summary: summaryPoints, taskId });
